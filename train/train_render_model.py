...
def Tensor2img(tensor_, channel_index):
    frame = tensor_[channel_index:channel_index + 3,
                    :, :].detach().squeeze(0).cpu().float().numpy()
...
if __name__ == "__main__":
    ...
    training_data_loader = DataLoader(
        dataset=train_set, num_workers=0, batch_size=opt.batch_size, shuffle=True)
    ...
    for epoch in range(opt.start_epoch, opt.non_decay + opt.decay + 1):
        ...
        try:
            for iteration, data in enumerate(training_data_loader):
                ...
                source_tensor, source_prompt_tensor = source_tensor[:,
                                                                    :3], source_tensor[:, 3:]
                # network forward
                fake_out = net_g(
                    source_tensor, source_prompt_tensor, ref_tensor)
                # down sample output image and real image
                fake_out_half = F.avg_pool2d(
                    fake_out, 3, 2, 1, count_include_pad=False)
                target_tensor_half = F.interpolate(
                    target_tensor, scale_factor=0.5, mode='bilinear')
                # (1) Update D network
                ...
                for i in range(len(perception_real)):
                    loss_g_perception += criterionL1(
                        perception_fake[i], perception_real[i])
                    loss_g_perception += criterionL1(
                        perception_fake_half[i], perception_real_half[i])
                loss_g_perception = (
                    loss_g_perception / (len(perception_real) * 2)) * opt.lamb_perception
                # gan dI loss
                ...
                message = "===> Epoch[{}]({}/{}): Loss_DI: {:.4f} Loss_GI: {:.4f} Loss_perception: {:.4f} lr_g = {:.7f} lr_d = {:.7f}".format(
                    epoch, iteration, len(training_data_loader), float(
                        loss_dI), float(loss_g_dI),
                ...
                if iteration % 200 == 0:
                    ...
                    inference_out = inference_out[0].cpu().permute(
                        1, 2, 0).float().detach().numpy().astype(np.uint8)
                    inference_in = (target_tensor[0, :3] * 255).cpu().permute(
                        1, 2, 0).float().detach().numpy().astype(np.uint8)
                    ...
                    inference_out = np.concatenate(
                        [inference_in, inference_in_prompt, inference_out, frame2, frame3], axis=1)
                    inference_out = cv2.cvtColor(
                        inference_out, cv2.COLOR_RGB2BGR)

                    log(train_logger, fig=inference_out,
                        tag="Training/epoch_{}_{}".format(epoch, iteration))

                    real_iteration = epoch * \
                        len(training_data_loader) + iteration
                    message1 = "Step {}/{}, ".format(
                        real_iteration, (epoch + 1) * len(training_data_loader))
                    message2 = ""
                    losses = [loss_dI.item(), loss_g_perception.item(),
                              loss_g_dI.item()]
                    train_logger.add_scalar(
                        "Loss/loss_dI", losses[0], real_iteration)
                    train_logger.add_scalar(
                        "Loss/loss_g_perception", losses[1], real_iteration)
                    train_logger.add_scalar(
                        "Loss/loss_g_dI", losses[2], real_iteration)
            ...
            print("-----------------")
            print(f"出现错误: {e}, 跳过此次训练...")
            print("-----------------")
            continue

        train_logger.add_scalar("Loss/{}".format("epoch_g_perception"), avg_loss_g_perception / len(training_data_loader), epoch)
        train_logger.add_scalar("Loss/{}".format("epoch_DI"), avg_Loss_DI / len(training_data_loader), epoch)
        train_logger.add_scalar("Loss/{}".format("epoch_GI"), avg_Loss_GI / len(training_data_loader), epoch)
        ...
        if epoch % opt.checkpoint == 0:
            ...
            model_out_path = os.path.join(
                opt.result_path, 'epoch_{}.pth'.format(epoch))
            states = {
                'epoch': epoch + 1,
